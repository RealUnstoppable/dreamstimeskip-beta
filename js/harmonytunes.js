...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // ⚡ Bolt: Global document event listeners extracted from the render loop to prevent O(N) memory leaks per render
    ...
    document.addEventListener('pointerup', (e) => {
        ...
        if (isDragging && dragItem) {
            ...
            const items = Array.from(queueContentArea.querySelectorAll('.queue-item')).filter(el => el.querySelector('.queue-more-btn'));
            const originalIdx = items.indexOf(dragItem);
            ...
            // It was just a tap/click! Open context menu if we clicked the more button
            const moreBtn = e.target.closest('.queue-more-btn');
            if (moreBtn && dragItem) {
               const items = Array.from(queueContentArea.querySelectorAll('.queue-item')).filter(el => el.querySelector('.queue-more-btn'));
               const originalIdx = items.indexOf(dragItem);
               if (originalIdx !== -1 && userQueue[originalIdx]) {
                   openQueueContextMenu(e, userQueue[originalIdx].id, originalIdx);
               }
            }
            dragItem = null;
        }
    ...
    function renderQueue() {
        ...
        // ⚡ Bolt: Batch DOM insertions using DocumentFragment to prevent O(N) reflows
        const fragment = document.createDocumentFragment();
        ...
        displayList.forEach((song, idx) => {
            ...
            if(isDraggable) {
                ...
                moreBtn.addEventListener('pointerdown', (e) => {
                    e.preventDefault();
                    isDragging = false;
                    dragItem = item; // Set early to handle fast clicks
