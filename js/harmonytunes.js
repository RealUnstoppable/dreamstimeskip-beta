...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // ⚡ Bolt: Hoisted pointermove and pointerup out of render loop to prevent exponential memory leaks
    ...
    document.addEventListener('pointerup', (e) => {
        ...
        if (isDragging && dragItem) {
            const dragItemToProcess = dragItem;
            isDragging = false;
            dragItem = null;
            dragItemToProcess.style.position = '';
            dragItemToProcess.style.zIndex = '';
            dragItemToProcess.style.transform = '';
            dragItemToProcess.classList.remove('dragging');
            if(queueContentArea) queueContentArea.style.cursor = '';

            // Calculate drop index based on position
            const items = Array.from(queueContentArea.querySelectorAll('.queue-item')).filter(el => el.querySelector('.queue-more-btn'));
            let droppedIdx = Array.from(items).indexOf(dragItemToProcess); // Dynamic index to avoid stale closure
            if (droppedIdx === -1) droppedIdx = 0; // fallback just in case
            ...
            renderQueue();
        } else if (!isDragging && dragItem) { // Tap/click!
            // Need the song info for context menu. We store in dataset
            ...
            if(idx !== -1) {
             ...
             dragItem = null; // Clean up just in case
        }
    });
    ...
    function renderQueue() {
        ...
        // ⚡ Bolt: DocumentFragment to batch inserts
        const fragment = document.createDocumentFragment();
        ...
        displayList.forEach((song, idx) => {
            ...
            if(isDraggable) {
                ...
                moreBtn.addEventListener('pointerdown', (e) => {
                    e.preventDefault();
                    isDragging = false;
                    dragItem = item; // Capture the item immediately for pointerup logic
                ...
                // Keep the 'click' logic on the button itself so it correctly scopes the click
