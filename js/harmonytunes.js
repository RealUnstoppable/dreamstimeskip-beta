...
document.addEventListener('DOMContentLoaded', () => {
    ...
    let dragOriginalIdx = null;

    // ⚡ Bolt: Global drag event listeners extracted from the inner render loop to prevent memory leaks
    ...
    document.addEventListener('pointerup', (e) => {
        ...
        if (isDragging && dragItem) {
            ...
            const items = Array.from(queueContentArea.querySelectorAll('.queue-item')).filter(el => el.querySelector('.queue-more-btn'));
            let droppedIdx = dragOriginalIdx;
            for (let i = 0; i < items.length; i++) {
                const rect = items[i].getBoundingClientRect();
                if (e.clientY < rect.top + rect.height / 2) {
                    droppedIdx = i;
            ...
            if (droppedIdx !== dragOriginalIdx && droppedIdx !== null && dragOriginalIdx !== null) {
                const movedSong = userQueue.splice(dragOriginalIdx, 1)[0];
                userQueue.splice(droppedIdx, 0, movedSong);
            }
            dragItem = null;
            dragOriginalIdx = null;
            renderQueue();
        } else if (dragItem && !isDragging) {
            // It was just a tap/click! Open context menu
            const songId = dragItem.dataset.songId;
            openQueueContextMenu(e, songId, dragOriginalIdx);
            dragItem = null;
            dragOriginalIdx = null;
        }
    });
        ...
        displayList.forEach((song, idx) => {
            ...
            if(isDraggable) {
                ...
                moreBtn.addEventListener('pointerdown', (e) => {
                    ...
                    // ⚡ Bolt: Rely on safe loop closure for original index
                    dragOriginalIdx = idx;
