...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // ⚡ Bolt: Global event listeners for drag-and-drop to prevent memory leaks in renderQueue
    ...
    document.addEventListener('pointerup', (e) => {
        ...
        if (isDragging && dragItem) {
            ...
            const items = Array.from(queueContentArea.querySelectorAll('.queue-item')).filter(el => el.querySelector('.queue-more-btn'));
            const originalIdx = parseInt(dragItem.dataset.index, 10);

            let droppedIdx = originalIdx;
            for (let i = 0; i < items.length; i++) {
                const rect = items[i].getBoundingClientRect();
                if (e.clientY < rect.top + rect.height / 2) {
                    droppedIdx = parseInt(items[i].dataset.index, 10);
                    break;
            ...
            if (!isNaN(originalIdx) && originalIdx !== -1 && droppedIdx !== originalIdx) {
