...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Attach global drag listeners once, outside the render loop, to prevent memory leaks
    ...
    document.addEventListener('pointerup', (e) => {
        ...
        if (isDragging && dragItem) {
            ...
            const idx = idxStr !== undefined ? parseInt(idxStr, 10) : Array.from(queueContentArea.querySelectorAll('.queue-item')).indexOf(item);
            ...
            if (droppedIdx !== idx && idx !== -1) {
                const movedSong = userQueue.splice(idx, 1)[0];
                userQueue.splice(droppedIdx, 0, movedSong);
            }
            renderQueue();
        } else if (!isDragging) {
            const queueMoreBtn = e.target.closest('.queue-more-btn');
            if (queueMoreBtn) {
                const item = queueMoreBtn.closest('.queue-item');
                if (item) {
                    const idxStr = item.dataset.index;
                    const songId = item.dataset.songId;
                    const idx = idxStr !== undefined ? parseInt(idxStr, 10) : Array.from(queueContentArea.querySelectorAll('.queue-item')).indexOf(item);
                    if (idx !== -1) {
                        openQueueContextMenu(e, songId || (userQueue[idx] ? userQueue[idx].id : null), idx);
                    }
                }
            }
        }
