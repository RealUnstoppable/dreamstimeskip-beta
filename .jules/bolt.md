...
## 2024-05-28 - Exponential Memory Leak in List Rendering
**Learning:** Attaching global event listeners (like `document.addEventListener('pointermove')`) inside a render loop (like `displayList.forEach`) causes an exponential memory leak. Every time the list re-renders, N new listeners are attached to the document. This severely degrades performance over time, especially in drag-and-drop implementations.
**Action:** Always declare global event listeners outside the render function/loop. Track the active element (e.g., `dragItem`) via state variables set during the element-specific `pointerdown` event.
