...
## 2026-06-27 - Fixing Memory Leaks in Render Loops
**Learning:** Attaching global event listeners like `document.addEventListener` inside dynamic render loops (e.g. `renderQueue()`) causes exponential memory leaks, as multiple listeners bind to the same event every time the UI updates.
**Action:** Always bind global event listeners once outside the render loop and rely on dynamic state lookup.
