...
## 2024-05-30 - Extracting Document Event Listeners from Loops
**Learning:** Attaching global event listeners like `document.addEventListener('pointermove')` inside a `.forEach()` loop (such as in `renderQueue()`) causes exponential listener accumulation on the `document` object. This creates a massive memory leak and significant lag during user interactions.
**Action:** Always hoist global event listeners outside of iteration loops to the module or scope level. Use state variables (like `isDragging`) and DOM queries (e.g., `Array.from().indexOf()`) to dynamically link the global listener to the specific item being interacted with.
