...
## 2024-05-27 - [Fix Privilege Escalation Vulnerability in Firestore Rules]
**Vulnerability:** The Firestore security rules for the `users` collection allowed users with matching UIDs to perform any generic `write` operation on their profile document. This enabled privilege escalation, as an attacker could modify sensitive fields (like `isAdmin`, `isBanned`, `plan`, `membershipLevel`, or `subscription`) to arbitrarily upgrade their permissions or bypass bans.
**Learning:** Relying on `request.auth.uid == userId` for generic `allow write` access on user profiles is insufficient, as it doesn't limit which fields the user is permitted to change.
**Prevention:** Always split `write` into `create`, `update`, and `delete`. In `update` blocks, strictly deny modification to sensitive fields using `!request.resource.data.diff(resource.data).affectedKeys().hasAny([...])`. In `create` blocks, validate that any sensitive fields either default to safe values or are omitted entirely.
