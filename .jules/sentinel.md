...
## 2024-06-17 - Privilege Escalation via User Profile Updates
**Vulnerability:** In `firestore.rules`, the `users/{userId}` collection allowed any user to write to their own profile with `allow write: if request.auth != null && request.auth.uid == userId;`. This allowed malicious users to self-assign the `isAdmin: true` field.
**Learning:** `allow write` combines `create`, `update`, and `delete`. Relying solely on `request.auth.uid == userId` for profile writes doesn't restrict the data payload, leading to privilege escalation when custom roles or flags like `isAdmin` are stored in the user document itself rather than custom claims.
**Prevention:** Split `allow write` into explicit `allow create`, `allow update`, and `allow delete` blocks. Use `!request.resource.data.diff(resource.data).affectedKeys().hasAny(['isAdmin'])` for updates, and verify default false values or missing fields for `create` blocks to prevent modification of sensitive fields by users.
