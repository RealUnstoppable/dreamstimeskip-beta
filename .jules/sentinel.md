...
## 2024-05-18 - [Fix Privilege Escalation in Firestore Users Rules]
**Vulnerability:** The Firestore security rules for the `/users/{userId}` collection used a generic `allow write: if request.auth != null && request.auth.uid == userId;`. This allowed any authenticated user to update their own profile document with arbitrary fields, including sensitive administrative flags like `isAdmin: true` or changing their own `membershipLevel` without paying.
**Learning:** Checking `request.auth.uid == userId` only verifies identity, it does not validate the shape or content of the data being written. Using generic `write` rules allows users to modify read-only or administrative fields.
**Prevention:** Split `allow write` into explicit `allow create`, `allow update`, and `allow delete`. In `update` blocks, restrict sensitive fields (like `isAdmin`, `isBanned`, `plan`, `membershipLevel`, `subscription`) using `!request.resource.data.diff(resource.data).affectedKeys().hasAny([...])`. In `create` blocks, enforce secure default values for these sensitive fields, using the `(!('fieldName' in request.resource.data) || request.resource.data.fieldName == false)` pattern to prevent rule evaluation errors if the field is omitted entirely from the payload.
