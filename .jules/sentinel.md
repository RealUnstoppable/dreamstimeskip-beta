...
## 2024-06-15 - Privilege Escalation via Generic Write Rules
**Vulnerability:** A critical privilege escalation vulnerability existed in `firestore.rules` where a user was granted generic `write` access to their own document under the `/users/{userId}` collection. This allowed any authenticated user to manually add or modify fields like `isAdmin: true` on their own profile, thereby gaining administrative permissions across the application.
**Learning:** Using a generic `allow write` rule instead of granular `allow create`, `allow update`, and `allow delete` rules can inadvertently expose sensitive data fields to user modification if validation checks are not comprehensive enough.
**Prevention:** Always separate `write` rules into `create` and `update` when managing user profiles. Use `!request.resource.data.keys().hasAny(['isAdmin'])` for creations, and `!request.resource.data.diff(resource.data).affectedKeys().hasAny(['isAdmin'])` for updates, to explicitly prevent modification of sensitive roles and flags.
