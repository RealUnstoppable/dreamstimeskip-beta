...
## 2025-02-15 - Privilege Escalation via User Profile Updates
**Vulnerability:** The `/users/{userId}` Firestore rules previously allowed users to arbitrarily update their own profile documents. This allowed users to grant themselves administrative privileges by simply sending a request setting `isAdmin: true` or upgrading their subscription status without paying by modifying `membershipLevel`, `plan`, or `subscription`.
**Learning:** Firestore rules that only check `request.auth.uid == userId` are insufficient when the user profile document contains sensitive role or permission attributes. These attributes are often relied upon elsewhere for access control (e.g., admin dashboards or pro features).
**Prevention:** Use `request.resource.data.diff(resource.data).affectedKeys().hasAny([...])` in `update` rules to explicitly deny updates to sensitive attributes. For document creation, use `request.resource.data.get(...)` or `request.resource.data.keys().hasAny(...)` to ensure sensitive values cannot be spoofed.
