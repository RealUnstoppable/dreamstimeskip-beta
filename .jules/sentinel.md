...
**Vulnerability:** The `/users/{userId}` Firestore rule used a generic `allow write` block that only checked `request.auth.uid == userId`. This allowed any authenticated user to update their own profile with administrative privileges, such as modifying their `isAdmin`, `plan`, or `membershipLevel` fields to gain unauthorized access to premium features or admin dashboards.
**Learning:** Relying solely on ownership checks (`request.auth.uid == userId`) in generic `allow write` blocks is insufficient for collections that contain sensitive, system-managed fields.
**Prevention:** Split `allow write` into explicit `allow create`, `allow update`, and `allow delete` blocks. In `update` rules, enforce that sensitive fields cannot be modified by the client using `!request.resource.data.diff(resource.data).affectedKeys().hasAny(['isAdmin', 'isBanned', 'plan', 'membershipLevel', 'subscription'])`. In `create` rules, enforce that these sensitive fields are either omitted or initialized to safe default values (e.g., `false` or `free`).
