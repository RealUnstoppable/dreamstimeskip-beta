...
## $(date +%Y-%m-%d) - Fix XSS Bypass in escapeHTML Utilities
**Vulnerability:** The `escapeHTML` utility functions across the application checked `if (typeof str !== 'string') str = str.toString();` before escaping. This failed to properly coerce arrays containing strings into single strings because `str.toString()` on an array returns a comma-separated string, but `replace` works directly on strings. As a result, an attacker could bypass sanitization by passing an array of strings like `['<script>alert(1)</script>']`.
**Learning:** `typeof str !== 'string'` combined with `str.toString()` is insufficient for sanitizing array inputs because arrays are implicitly coerced to strings during template literal interpolation later on, but bypass the explicit regex replacement logic.
**Prevention:** Always explicitly cast all inputs to strings using `String(str)` inside the `escapeHTML` utility before applying regex replacements.
