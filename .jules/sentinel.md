...
## 2026-05-27 - [Fix Firestore Privilege Escalation]
**Vulnerability:** The Firestore rules for the `/users/{userId}` collection allowed any authenticated user to update any field on their own document. This allowed users to self-escalate privileges by updating fields like `isAdmin`, `isBanned`, `plan`, `membershipLevel`, or `subscription` to unauthorized values directly from the client.
**Learning:** `allow write: if request.auth.uid == userId` is insufficient for securing user documents if those documents contain sensitive roles or server-managed states.
**Prevention:** Always split `write` into `create`, `update`, and `delete`. In `update` blocks, strictly restrict sensitive fields using `!request.resource.data.diff(resource.data).affectedKeys().hasAny(['isAdmin', 'isBanned', 'plan', 'membershipLevel', 'subscription'])`. In `create` blocks, explicitly verify that sensitive fields are either missing or set to safe default values.
