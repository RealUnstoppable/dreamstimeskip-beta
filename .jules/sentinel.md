...
## 2026-06-08 - [Fix Authorization Bypass in Firebase Security Rules]
**Vulnerability:** The `firestore.rules` file contained duplicate `match` blocks for the same collection paths (e.g., `/reviews`, `/product_stats`, and `/product_reviews`). Since Firebase evaluates overlapping rules with a logical OR, the less restrictive duplicate blocks at the bottom of the file silently bypassed the intended stricter validations defined earlier in the file.
**Learning:** Duplicate `match` blocks in `firestore.rules` for the same collection path are evaluated with a logical OR. This means that if any block grants access, access is granted, effectively nullifying stricter rules placed in other overlapping blocks.
**Prevention:** Strictly avoid duplicate `match` blocks for the same collection path. Consolidate all rules for a specific path into a single `match` block to ensure intended restrictions are reliably enforced.
