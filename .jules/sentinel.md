...
## 2024-06-25 - [Fix Privilege Escalation via Firestore Rules]
**Vulnerability:** The Firestore rules for the `/users/{userId}` collection used a generic `allow write: if request.auth.uid == userId` check. This allowed users to update arbitrary fields on their profile, such as `isAdmin`, `isBanned`, `membershipLevel`, `plan`, and `subscription`, leading to privilege escalation.
**Learning:** A simple owner check (`uid == userId`) in Firestore is not sufficient for securing documents if the document contains sensitive role-based or system-managed fields.
**Prevention:** Split the `write` permission into `create`, `update`, and `delete`. In the `update` block, restrict modification of sensitive fields using `!request.resource.data.diff(resource.data).affectedKeys().hasAny(['isAdmin', 'isBanned', 'plan'])`. In the `create` block, ensure that these sensitive fields are either omitted or initialized to safe default values using `!('field' in request.resource.data) || request.resource.data.field == false`.
