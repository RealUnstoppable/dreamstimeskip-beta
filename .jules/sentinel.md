...
## 2024-05-18 - [Fix Privilege Escalation (IDOR) in Firestore Rules]
**Vulnerability:** The Firestore security rules for the `users` collection allowed any authenticated user to update arbitrary fields on their own profile because it simply used `allow write: if request.auth != null && request.auth.uid == userId;`. This meant a user could maliciously escalate their privileges by setting fields like `isAdmin`, `plan`, `membershipLevel`, or `isBanned` directly in their client-side `setDoc` or `updateDoc` calls.
**Learning:** Broad `allow write` rules in Firestore (or generic database update handlers) that check only identity (`userId`) but not payload content leave applications vulnerable to Mass Assignment/Privilege Escalation attacks.
**Prevention:** Always split `write` into `create`, `update`, and `delete`. In `update` rules, explicitly block modifications to sensitive fields using `!request.resource.data.diff(resource.data).affectedKeys().hasAny([...])`. In `create` rules, enforce strict default values (or absence) for those same sensitive fields.
