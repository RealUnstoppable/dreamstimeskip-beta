...
## 2026-06-15 - [Fix Overlapping Firestore Rules Bypass]
**Vulnerability:** The `firestore.rules` file contained duplicate, overlapping `match` blocks for `/reviews/{reviewId}`, `/product_stats/{productId}`, and `/product_reviews/{reviewId}`. The secondary `product_reviews` block omitted `userEmail` validation (spoofing protection) and allowed permissive evaluations. Since Firebase evaluates overlapping rules with a logical OR, the less restrictive rules bypassed the strict validation checks intended by the primary blocks. Additionally, the primary block incorrectly allowed `update` and `delete` operations which could corrupt aggregate ratings since the backend only tracked creations via `onDocumentCreated`.
**Learning:** Duplicate or overlapping `match` blocks in Firebase rules act as a logical OR. Permissive blocks silently override stricter validations located elsewhere. Client-side state aggregation based on `onDocumentCreated` can be corrupted if clients are permitted to bypass it via unprotected `update` and `delete` rules.
**Prevention:** Always deduplicate `match` blocks and combine conditions logically into a single block. Disallow arbitrary update/delete operations when backend aggregations strictly rely on creation triggers.
