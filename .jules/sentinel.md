...
## 2024-07-12 - [Fix Privilege Escalation in Firestore Rules]
**Vulnerability:** The Firestore rules for the `users` collection allowed users to arbitrarily write to their own documents using a blanket `allow write: if request.auth != null && request.auth.uid == userId;` rule. This allowed privilege escalation, as users could modify sensitive fields like `isAdmin`, `isBanned`, `plan`, `membershipLevel`, and `subscription` to bypass application restrictions or gain unauthorized privileges.
**Learning:** A generic `write` rule that only checks the user ID is insufficient for securing user profiles when those documents contain authorization or billing metadata.
**Prevention:** Always split `write` into `create`, `update`, and `delete`. In the `update` block, explicitly restrict modification of sensitive fields using `!request.resource.data.diff(resource.data).affectedKeys().hasAny(['sensitiveField'])`. In the `create` block, ensure these fields are either omitted or set to safe default values.
