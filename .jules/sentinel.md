...
## 2024-05-12 - DOM XSS in `tracker.html` Report Generation
**Vulnerability:** A DOM-based Cross-Site Scripting (XSS) vulnerability existed in `tracker.html` within the `generateReport` function. User-controlled data (`r.name`, `d.name`, `d.val`, etc.) from the tracker state was concatenated into HTML strings and injected directly into the DOM using `.innerHTML` without sanitization. An attacker could exploit this by creating a crafted routine or inventory item containing malicious scripts, which would execute when the report is generated.
**Learning:** Always validate and sanitize data that is rendered to the DOM, particularly when dealing with HTML strings. Never trust user input, even if it comes from the application's state. When generating dynamic content, using `.textContent` is generally safer than `.innerHTML`. If `.innerHTML` must be used, ensure all dynamic values are properly escaped.
**Prevention:** Use the existing `escapeHTML()` function to sanitize all dynamic strings before concatenating them into HTML strings that will be injected via `.innerHTML`. Extend the fix to all instances within the same context where untrusted data could be similarly abused.
