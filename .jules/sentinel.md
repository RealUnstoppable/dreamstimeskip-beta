...
## $(date +%Y-%m-%d) - [Fix Privilege Escalation in Firestore Rules]
**Vulnerability:** The default `firestore.rules` for the `users` collection allowed any authenticated user to write entirely to their own document without field-level restrictions (`allow read, write: if request.auth != null && request.auth.uid == userId;`). This permitted an attacker to grant themselves admin privileges or paid subscriptions by updating sensitive fields like `isAdmin` or `plan`.
**Learning:** Checking that `request.auth.uid == userId` is only sufficient for document ownership, not for field-level authorization. Sensitive fields must be explicitly protected.
**Prevention:** Always split `allow write` into `allow create`, `allow update`, and `allow delete`. Use `request.resource.data.diff(resource.data).affectedKeys().hasAny(['sensitiveField'])` to restrict updates, and `request.resource.data.get('sensitiveField', default)` to enforce safe defaults on creation.
