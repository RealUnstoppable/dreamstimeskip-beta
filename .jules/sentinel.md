...
## 2025-02-15 - [Firebase Rule Precedence Bug]
**Vulnerability:** Authorization bypass due to overlapping `match` blocks for `/product_reviews/{reviewId}` and other duplicated paths in `firestore.rules`. The less restrictive duplicated blocks bypassed the stricter blocks.
**Learning:** Overlapping `match` blocks in Firebase evaluate with a logical OR. If one block grants access, Firebase ignores other stricter blocks that might deny access, causing the less restrictive rules to silently bypass stricter validations.
**Prevention:** Strictly avoid duplicate `match` blocks for the same collection path. If modifying security rules, ensure only a single, comprehensive `match` block governs a path to guarantee all validations run reliably.
