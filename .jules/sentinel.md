...
## 2026-05-12 - [Fix DOM-based XSS in Frontend Views]
**Vulnerability:** Several frontend files (`js/auth.js`, `js/shop.js`, `js/harmonytunes.js`) were directly injecting potentially user-controlled or dynamic database fields (like names, titles, and descriptions) into the DOM using `.innerHTML` inside template literals without sanitization.
**Learning:** In vanilla HTML/JS applications, avoid directly interpolating dynamic or untrusted data into `.innerHTML` without escaping.
**Prevention:** Always use an `escapeHTML` utility to sanitize untrusted user input before rendering it in the DOM, or rely on `textContent` or `innerText` instead.
