...
## 2024-05-24 - Firestore Privilege Escalation via generic write
**Vulnerability:** The Firestore `firestore.rules` file had a generic `allow write: if request.auth != null && request.auth.uid == userId;` rule for the `/users/{userId}` collection. This allows any authenticated user to send arbitrary writes via the frontend SDK to update fields such as `isAdmin`, `isBanned`, `plan`, and `subscription` to escalate their privileges.
**Learning:** Generic `allow write` block is insufficient for documents that contain sensitive internal system states mixed with public/user-modifiable data. Firestore rules crash if `resource` doesn't exist during creation, so `allow create` and `allow update` must be separated.
**Prevention:** Always separate `allow create` and `allow update`. In `create` rules, deny sensitive fields with `request.resource.data.get('field', false) == false` or similar default checks. In `update` rules, enforce `!request.resource.data.diff(resource.data).affectedKeys().hasAny(['isAdmin', ...])` to ensure users cannot tamper with privileged fields.
