...
**Vulnerability:** The Firestore rules allowed any authenticated user to create or update their own user profile document with elevated privileges like `isAdmin: true` because it relied on `allow read, write: if request.auth != null && request.auth.uid == userId;` without any payload restrictions.
**Learning:** Generic `allow write` rules provide full control over document creation and updates, allowing users to escalate privileges if sensitive fields are stored in the user document and not restricted. `resource` is null during document creation, so `resource.data.diff()` cannot be used in a generic `write` block or an `allow create` block without crashing the rule.
**Prevention:** Split `allow write` into `allow create`, `allow update`, and `allow delete`. In `allow update`, explicitly restrict sensitive fields using `!request.resource.data.diff(resource.data).affectedKeys().hasAny([...])`. In `allow create`, verify default values for sensitive fields using the `(!('fieldName' in request.resource.data) || request.resource.data.fieldName == <default>)` pattern to prevent rule evaluation errors if the field is omitted entirely from the payload.
