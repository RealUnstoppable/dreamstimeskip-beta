...
## 2024-05-24 - Overlapping Firestore Rules Vulnerability
**Vulnerability:** Duplicate and overlapping Firestore rule `match` blocks for `/reviews`, `/product_stats`, and `/product_reviews` caused stricter validations to be silently bypassed.
**Learning:** Firebase evaluates overlapping `match` rules with a logical OR. This means that if any single `match` block grants access (e.g., being less restrictive), the access is granted, effectively nullifying stricter rules defined in other blocks for the same path.
**Prevention:** Always ensure there is only one `match` block per collection path in `firestore.rules`. Regularly audit the file to remove any duplicate blocks, keeping only the most restrictive and comprehensive set of rules.
