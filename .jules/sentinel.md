...
## 2026-05-27 - [Fix Privilege Escalation via Client-Side Firestore Rules]
**Vulnerability:** The Firestore security rules for the `/users/{userId}` collection allowed any authenticated user to write to their own profile document (`allow write: if request.auth != null && request.auth.uid == userId;`). Because there was no field-level validation, a malicious user could modify their own `isAdmin` or `plan` fields to escalate privileges (e.g., bypassing subscription checks or gaining admin dashboard access).
**Learning:** Broad `allow write` rules on user documents are dangerous if the document stores authorization metadata or state managed by the backend (like subscription tiers or admin flags).
**Prevention:** Explicitly block modifications to sensitive fields using `!request.resource.data.diff(resource.data).affectedKeys().hasAny(['isAdmin', 'isBanned', 'plan', 'subscription', 'membershipLevel'])` or separate sensitive authorization data into a private collection that clients cannot write to at all.
