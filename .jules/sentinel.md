...
**Vulnerability:** The `firestore.rules` file contained duplicate `match` blocks for collections like `/product_reviews/{reviewId}`. Because Firebase evaluates overlapping rules with a logical OR, the less restrictive block silently bypassed the stricter validations (e.g., email verification via `request.auth.token.email`) present in the earlier block.
**Learning:** Duplicate `match` blocks for the same path in Firestore rules create serious vulnerabilities, as less restrictive rules will override stricter ones.
**Prevention:** Strictly avoid duplicate `match` blocks for the same collection path. Consolidate rules into a single block per path.
