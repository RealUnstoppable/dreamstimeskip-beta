...
## 2026-06-25 - [Fix Insecure Access Control in ticket service]
**Vulnerability:** The client-side application was directly calling `updateDoc` on the `support_tickets` collection to add admin replies, relying entirely on Firestore security rules for protection.
**Learning:** While Firestore rules provide some protection (e.g., `isAdmin()`), direct client-side administrative database calls are considered insecure in this project's architecture and violate the requirement to route all admin actions through a backend Cloud Function. This ensures better auditability, avoids exposing administrative schemas to the client, and centralizes access control.
**Prevention:** All administrative database mutations (like updating support tickets or banning users) must be executed by making an authenticated HTTP request to the secure backend Firebase Cloud Function (`adminAction`), passing the user's ID token in the `Authorization` header.
