...
## 2025-02-12 - [Fix Privilege Escalation in User Profiles]
**Vulnerability:** The Firestore security rules allowed users to write arbitrary data to their own profile document (`/users/{userId}`). This permitted a privilege escalation attack where a malicious user could set `isAdmin: true`, `isBanned: false`, or modify their subscription status (`plan`, `membershipLevel`, `subscription`).
**Learning:** Generic `allow write: if request.auth.uid == userId;` rules on user profile collections are dangerous if those documents contain sensitive authorization fields, as users have full write access to their own records.
**Prevention:** Always split `write` into `create`, `update`, and `delete`. Use `!request.resource.data.diff(resource.data).affectedKeys().hasAny([...])` to prevent modification of sensitive fields during updates, and explicitly enforce safe default values (e.g., `false` or missing) for those fields during creation.
