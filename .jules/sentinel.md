...
## 2024-05-18 - [Fix Hardcoded Stripe Secret]
**Vulnerability:** The Stripe Node.js SDK initialization in `server.js` contained a hardcoded empty string (`const stripe = Stripe(""); // 🔴 replace`), which is an anti-pattern. While it was a placeholder, developers often mistakenly commit live secrets here.
**Learning:** Hardcoded credentials should never be committed into source code as they can be easily leaked, especially if the repository is made public. Environment variables provide a secure way to inject configuration at runtime.
**Prevention:** Always initialize external services requiring API keys or secrets dynamically using `process.env` (e.g., `process.env.STRIPE_SECRET_KEY`) to enforce secure credential management.
