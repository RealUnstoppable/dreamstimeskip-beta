...
**Vulnerability:** The Firestore rules for the `users` collection allowed any authenticated user to write arbitrary data to their own profile, including sensitive fields like `isAdmin`, `plan`, and `subscription`.
**Learning:** `allow write` is dangerous when documents contain sensitive fields. `resource.data.diff()` cannot be used in generic write blocks because `resource` is null during creation.
**Prevention:** Split write rules into `create`, `update`, and `delete`. Restrict `update` using `!request.resource.data.diff(resource.data).affectedKeys().hasAny([...])` and enforce default/missing values during `create` with `!('field' in request.resource.data) || request.resource.data.field == false`.
