...
## 2026-06-03 - [Fix Privilege Escalation Vulnerability]
**Vulnerability:** A privilege escalation vulnerability was found in the `users` collection in `firestore.rules`. Any user could write arbitrary values to their user document, including setting `isAdmin: true` or unsetting `isBanned`, allowing them to bypass security rules on the `support_tickets` collection or ban checks.
**Learning:** In Firestore rules, the `write` rule doesn't allow granular control over which fields can be modified. It's critical to split it into `create`, `update`, and `delete` blocks, especially for documents containing sensitive authorization data.
**Prevention:** Split `allow write` into explicit `create`, `update`, and `delete` blocks. For `update`, use `!request.resource.data.diff(resource.data).affectedKeys().hasAny(['isAdmin', 'isBanned'])` to prevent users from modifying sensitive fields. For `create`, use `(!('fieldName' in request.resource.data) || request.resource.data.fieldName == false)` to ensure default values cannot be manipulated.
