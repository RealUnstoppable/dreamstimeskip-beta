...
## 2025-05-15 - Duplicate Firestore Rules Bypass Intent
**Vulnerability:** Duplicate overlapping rules exist in `firestore.rules` for `/reviews`, `/product_stats`, and `/product_reviews` collections. Since Firebase evaluates rules with a logical OR, the more permissive rules silently override the stricter ones.
**Learning:** Duplicate definitions of the same path silently create bypasses, making stricter constraints entirely ineffective.
**Prevention:** Ensure no duplicate paths exist in Firestore rules files and use tools or careful review to identify duplicates.
