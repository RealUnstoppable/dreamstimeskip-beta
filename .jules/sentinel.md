...
## 2024-05-18 - [Fix Privilege Escalation in User Profiles]
**Vulnerability:** The Firestore security rules for the `/users/{userId}` collection allowed users full `write` access to their own document. This allowed any user to update their document to set `isAdmin: true` or update their `plan` or `subscription` fields, leading to privilege escalation.
**Learning:** Never grant blanket write access to user profile documents if they contain authorization or subscription states used for access control.
**Prevention:** Explicitly restrict which fields a user can modify on `create` and `update` using `request.resource.data.get('field', defaultValue)` and `request.resource.data.diff(resource.data).affectedKeys().hasAny([...])`.
