...
## 2025-06-12 - Missing ARIA Labels on Implicitly Labelled Form Inputs
**Learning:** In projects heavily utilizing flexbox utilities and placeholder text, inputs (like search bars, custom preset fields, and promo code inputs) often lack dedicated `<label>` tags to save visual space. While this maintains a clean UI, it severely impacts screen reader accessibility if `aria-label` attributes are omitted.
**Action:** When auditing or implementing new components that lack visual labels or rely on `placeholder` attributes (e.g., `#global-search` or `#promo-code`), always proactively add descriptive `aria-label`s to ensure the purpose of the input is announced to assistive technologies.
